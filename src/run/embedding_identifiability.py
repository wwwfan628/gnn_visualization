...
from src.utils.train_embedding_id import train_citation, train_ppi, train_regression_citation,train_regression_ppi
...
def compute_nodes_attribution_citation(net, graph, features, layer, args):
...
# def compute_nodes_attribution_ppi(net, train_dataset, layer, args):
#     max_contributed_input_nodes = np.array([])
#     inputs = features.clone().detach().requires_grad_(True).to(device)
#     net.eval()
#     embedding = net(graph,inputs)[-layer]
#     inputs_gradient = np.zeros([embedding.shape[1], inputs.shape[0], inputs.shape[1]])
...
def main(args):
    ...
    embedding_id_rates_df = pd.DataFrame(np.zeros([6, len(models)]), index=np.arange(1,7), columns=models)
    node_repeatability_rates_df = pd.DataFrame(np.zeros([6, len(models)]), index=np.arange(1, 7), columns=models)
    max_contributed_neighbour_recovered_rates_df = pd.DataFrame(np.zeros([6, len(models)]), index=np.arange(1, 7), columns=models)
    for gcn_model_layer in np.arange(2, 7):
        ...
        embedding_id_rates = np.zeros([6,args.repeat_times])
        # node repeatablity rates for one model
        node_repeatability_rates = np.zeros([6,args.repeat_times])
        # if not identifiable, the rates of identifying the neighbourhood node which contributes at most
        max_contributed_neighbour_recovered_rates = np.zeros([6,args.repeat_times])
        for repeat_time in np.arange(args.repeat_times):
            ...
            if args.dataset in 'cora, citeseer, pubmed':
                ...
                identifiable_nodes_1layer = np.array([])
                embedding_id_rates_layer = np.zeros(len(train_dataset))
                node_repeatability_rates_layer = np.zeros(len(train_dataset))
                max_contributed_neighbour_recovered_rates_layer = np.zeros(len(train_dataset))
            for intermediate_layer in np.arange(1, gcn_model_layer+1):
                ...
                if args.info == 'self-identity':
                    if args.dataset in 'cora, citeseer, pubmed':
                        ...
                        # max_contributed_nodes = compute_nodes_attribution_citation(gcn, g, features, intermediate_layer, args)
                        max_contributed_nodes = np.zeros(g.number_of_nodes())
                ...
                if args.info == 'self-identity':
                    if args.dataset in 'cora, reddit-self-loop, citeseer, pubmed':
                        ...
                        train_regression_ppi(reg, train_dataloader, valid_dataloader)
                ...
                if args.info == 'self-identity':
                    if args.dataset in 'cora, citeseer, pubmed':
                        ...
                        if args.knn == 1:
                            print("********** NEAREST NEIGHBOUR TO FIND CORRESPONDING INPUT **********")
                            for graph_id, data in enumerate(train_dataset):
                                num_identifiable = 0.0
                                num_repeatable = 0.0 # number of repeatable nodes of current graph
                                num_nn_is_max_contributed_neighbour = 0.0
                                identifiable_nodes_current_layer_set = set()
                                identifiable_nodes_1layer_set = set()
                                nn_list = np.zeros(data[0].number_of_nodes()) # indices of the found nearest neighbourhood of nodes
                                ...
                                for node_ind in range(data[0].number_of_nodes()):
                                    if node_ind == nn_list[node_ind]:  # whether identifiable
                                        num_identifiable += 1
                                        identifiable_nodes_current_layer_set.add(node_ind)
                                        if intermediate_layer==1:
                                            identifiable_nodes_1layer_set.add(node_ind)
                                if intermediate_layer==1:
                                    identifiable_nodes_1layer = np.append(identifiable_nodes_1layer, identifiable_nodes_1layer_set)
                                for node_ind in identifiable_nodes_current_layer_set:
                                    if node_ind in identifiable_nodes_1layer[graph_id]:
                                        num_repeatable+=1
                                node_repeatability_rates_layer[graph_id] = num_repeatable / data[0].number_of_nodes()
                                embedding_id_rates_layer[graph_id] = num_identifiable / data[0].number_of_nodes()
                                print('Graph id {} | identifiability_rate = {} %'.format(graph_id, embedding_id_rates_layer[graph_id] * 100))
                                print('Graph id {} | node_repeatability_rate = {} %'.format(graph_id, node_repeatability_rates_layer[graph_id] * 100))
                            embedding_id_rates[intermediate_layer - 1, repeat_time] = np.mean(embedding_id_rates_layer)
                            node_repeatability_rates[intermediate_layer - 1, repeat_time] = np.mean(embedding_id_rates_layer)
                            max_contributed_neighbour_recovered_rates[intermediate_layer - 1, repeat_time] = 0
                ...
                if args.info == 'self-identity':
                    if args.dataset in 'cora, reddit-self-loop, citeseer, pubmed':
                        ...
                        for node_ind in range(g.number_of_nodes()):
                            if args.knn == 1:
                                ...
                                elif max_contributed_nodes[node_ind] == nn_list[node_ind]:
                    ...
                    print('max_contributed_neighbour_recovered_rate = {} %'.format(max_contributed_neighbour_recovered_rate * 100))
                    embedding_id_rates[intermediate_layer-1, repeat_time] = identifiability_rate
                    node_repeatability_rates[intermediate_layer-1, repeat_time] = node_repeatability_rate
        ...
        node_repeatability_rates_df[df_column_ind] = np.mean(node_repeatability_rates, axis=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_gcn_layer', type=int, default=6, help='the maxmal gcn models\'s layer')
