...
def main(args):
    ...
    if args.dataset in 'cora, citeseer, pubmed':
        ...
        accuracy_id_df = pd.DataFrame(np.mean(accuracy_id, axis=0), index=models, columns=['accuracy'])
        accuracy_unid_df = pd.DataFrame(np.mean(accuracy_unid, axis=0), index=models, columns=['accuracy'])
        ...
        save_subpath = 'accuracy_id_' + args.dataset + '_' + str(args.knn) + '-nn' + '.csv'
        save_path = os.path.join(outputs_subdir, save_subpath)
        accuracy_id_df.to_csv(save_path)
        save_subpath = 'accuracy_unid_' + args.dataset + '_' + str(args.knn) + '-nn' + '.csv'
        save_path = os.path.join(outputs_subdir, save_subpath)
        accuracy_unid_df.to_csv(save_path)
        ...
        models = []  # dataframe column index
        for gcn_model_layer in np.arange(1, args.max_gcn_layers + 1):
            model_name = str(gcn_model_layer) + '-layer ' + 'GCN'
            models.append(model_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_gcn_layers', type=int, default=10, help='the maxmal gcn models\'s layer, not larger than 10')
